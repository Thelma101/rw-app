...
const Quiz: React.FC = () => {
  const { questions, currentQuestionIndex, answers, timeStarted, setQuestions, setAnswer, submitQuiz } = useQuizStore();
  ...
  const handleSubmit = async () => {
    try {
      ...
      const answersWithCorrectness = questions.map((question, index) => ({
        ...
        isCorrect: question.correct_answer === answers[index]
      }));

      const results = await submitQuiz(answersWithCorrectness, timeTaken);
      console.log('Quiz submitted successfully:', results);
    } catch (error) {
